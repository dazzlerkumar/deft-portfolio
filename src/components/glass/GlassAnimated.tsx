...
import { useMagneticHover, useScrollGlassMorph } from '@/hooks/useMagneticHover';
...
interface GlassAnimatedProps extends GlassComponentProps {
  /** Animation type */
  animationType?: 'lift' | 'morph' | 'button' | 'card' | 'scroll' | 'magnetic';
...
export const GlassAnimated = forwardRef<HTMLElement, GlassAnimatedProps>(
  ({
    ...
    const { config, cssVars, className: glassClassName } = getGlassConfiguration({
      variant,
      theme,
      className,
      ...props,
    ...
    const getAnimationVariants = () => {
      ...
      switch (animationType) {
        ...
        case 'magnetic':
          return {
            rest: { scale: 1 },
            hover: { scale: 1.05, transition: glassTransitions.hover },
          };
        default:
          return glassLiftVariants;
      }
    ...
    const combinedRef = (element: HTMLElement | null) => {
      if (magnetic && magneticHover.ref) {
        (magneticHover.ref as any).current = element;
      }
    ...
    const dynamicStyles = {
      ...
      ...(scrollAnimation && {
        ...
        '--glass-saturation': `${150 + scrollGlass.scrollProgress * 50}%`,
      }),
    ...
    const animationProps = {
      ...
      animate: scrollAnimation ? (isInView ? 'visible' : 'hidden') : 'rest',
